...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    // Highly optimized iteration parameters for 60fps performance without GPU lock
    const qualitySettings = {
      low: { iterations: 20, waveIterations: 1, pixelRatio: 0.5, precision: 'mediump', stepMultiplier: 1.6 },
      medium: { iterations: 32, waveIterations: 2, pixelRatio: 0.65, precision: 'mediump', stepMultiplier: 1.3 },
      high: {
        iterations: 44,
        waveIterations: 2,
        pixelRatio: Math.min(typeof window !== 'undefined' ? window.devicePixelRatio : 1, 1.25),
        precision: 'mediump',
        stepMultiplier: 1.15
      }
    };
    ...
    const animate = currentTime => {
      if (!isVisibleRef.current) {
        // Skip heavy WebGL computation when scrolled away
